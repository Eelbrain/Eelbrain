...
class App(wx.App):
    ...
    def OnInit(self):
        ...
        # register in IPython
        # Test for a running shell rather than for the module: third-party imports (e.g., seaborn -> ipywidgets) can load IPython without a shell
        shell = sys.modules['IPython'].get_ipython() if 'IPython' in sys.modules else None
        if CONFIG['prompt_toolkit'] and shell is not None and packaging.version.parse(sys.modules['IPython'].__version__) >= packaging.version.parse('5'):
