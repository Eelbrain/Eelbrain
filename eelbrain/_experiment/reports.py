...
def report_subject_info(p, ds, model):
    ...
    s_ds = p.show_subjects(asds=True)
    if 'n' in ds:
        if model:
...
def report_test_info(p, section, ds, test, res, data, include=None, model=True):
    ...
    test_obj = p._tests[test] if isinstance(test, str) else test
    ...
    info = res.info_list()
    section.append(info)
    section.append(report_subject_info(p, ds, test_obj.model))
...
def _report_title(p) -> str:
    return p.format('{raw_basename}_{test_basename}_epoch-{epoch}_test-{test}_options-{test_options}')
...
def _append_source_report(p, report, ctx: DerivativeContext) -> None:
    ds, res = p._load_test_context(ctx, True, True)
    report_test_info(p, report.add_section("Test Info"), ds, ctx.option('test'), res, ctx.option('data'), ctx.option('include'))
    parc = ctx.option('parc')
    mask = ctx.option('mask')
    if parc:
        section = report.add_section(parc)
        report_parc_image(p, section, f"Labels in the {parc} parcellation.")
    elif mask:
        section = report.add_section(f"Whole Brain Masked by {mask}")
        report_parc_image(p, section, f"Mask: {mask.capitalize()}")
...
def _append_two_stage_report(p, report, ctx: DerivativeContext) -> None:
    test_obj = p._tests[ctx.option('test')]
    return_data = bool(test_obj.model)
    result = p._load_test_context(ctx, return_data, True)
    if return_data:
        group_ds, rlm = result
    else:
        group_ds = None
...
class SourceReportDerivative(ResultOutputDerivative[str]):
    ...
    def build(self, ctx: DerivativeContext) -> str:
        ...
        if isinstance(p._tests[ctx.option('test')], TwoStageTest):
            _append_two_stage_report(p, report, ctx)
        else:
            _append_source_report(p, report, ctx)
...
class ROIReportDerivative(ResultOutputDerivative[str]):
    ...
    def build(self, ctx: DerivativeContext) -> str:
        ...
        info_section = report.add_section("Test Info")
        report_test_info(p, info_section, res.n_trials_ds, p._tests[ctx.option('test')], res.res[first_label], ctx.option('data'))
        section = report.add_section(ctx.option('parc'))
        report_parc_image(p, section, f"ROIs in the {ctx.option('parc')} parcellation.", res.subjects)
...
class EEGReportDerivative(ResultOutputDerivative[str]):
    ...
    def build(self, ctx: DerivativeContext) -> str:
        ...
        report = fmtxt.Report(_report_title(p))
        info_section = report.add_section("Test Info")
        report_test_info(p, info_section, ds, ctx.option('test'), res, ctx.option('data'), ctx.option('include'))
...
class EEGSensorsReportDerivative(ResultOutputDerivative[str]):
    ...
    def build(self, ctx: DerivativeContext) -> str:
        ...
        sensor_map.close()
        test_kwargs = p._test_kwargs_context(ctx, ('time', 'sensor'), None)
        ...
        report_test_info(p, info_section, ds, ctx.option('test'), results[0], ctx.option('data'))
        return _save_report(report, dst, ('eelbrain', 'mne', 'scipy', 'numpy'), ctx.option('samples'))
...
class CoregReportDerivative(Derivative[str]):
    ...
    def _subject_states(self, ctx: DerivativeContext) -> list[dict[str, Any]]:
        p = ctx.pipeline
    ...
    def fingerprint(self, ctx: DerivativeContext) -> dict[str, Any]:
        p = ctx.pipeline
        subjects = []
        for state in self._subject_states(ctx):
            with p._temporary_state:
                p.set(**state)
