...
class DerivativeRegistry:
    ...
    def _reconstruct_request(self, manifest: ArtifactManifest) -> Request[Any] | None:
        ...
        with warnings.catch_warnings():
            ...
            return self.resolve(manifest.derivative, state=dict(manifest.resolve_state), options=dict(manifest.resolve_options))
    ...
    def scan_cache(self, revalidate: bool = True) -> GCReport:
        ...
        with self._readonly_context():
            for child in sorted(self.cache_dir.iterdir()):
                ...
                elif child.name in self._nodes:
                    self._scan_node_dir(self._nodes[child.name], child, report, scanned, revalidate)
                else:
            ...
            self._propagate_stale_dependencies(report, scanned)
        return report
    ...
    def _scan_node_dir(
        ...
        for dirpath, dirnames, filenames in os.walk(node_dir):
            ...
            for manifest_name in sorted(manifest_names):
                ...
                self._classify_manifest(node, directory / manifest_name, report, scanned, revalidate)
            if is_versioned_input:
                keep, stale_references = self._scan_reference_files(directory, filenames)
            else:
            ...
            for name in sorted(set(filenames) - set(manifest_names) - artifact_names - keep):
                ...
                if name.endswith('.tmp'):
                    ...
                    self._scan_disambiguation_sidecar(node, path, report)
    ...
    def _disambiguation_target_exists(self, base_path: Path, suffix: str) -> bool:
    ...
    def _scan_disambiguation_sidecar(self, node: DependencyNode[Any], sidecar_path: Path, report: GCReport) -> None:
        ...
        mapping = self._read_cache_disambiguation(base_path)
        if not mapping:
            ...
            return
        dead = tuple(digest for digest, suffix in sorted(mapping.items()) if not self._disambiguation_target_exists(base_path, suffix))
        if not dead:
            return
    ...
    def _classify_manifest(
        ...
        def reconstruct() -> tuple[Request[Any] | None, str | None]:
            try:
                ctx = self._reconstruct_request(manifest)
        ...
        entry = None
        owner = self._nodes.get(manifest.derivative)
        if owner is None:
            ...
            if manifest.schema_version != MANIFEST_SCHEMA_VERSION:
                ...
                elif not self.is_cache_artifact(ctx.artifact_path):
                    entry = classify(GCCategory.UNVERIFIABLE, 'unexpected file next to an external-artifact mirror manifest')
                elif not self._same_path(ctx.artifact_path, artifact_path):
                    entry = classify(GCCategory.SUPERSEDED_KEY, f"current request resolves to {self.describe_artifact_path(ctx.artifact_path)}")
                elif revalidate:
                    try:
            ...
            if ctx is None:
                entry = classify(GCCategory.UNVERIFIABLE, failure)
            elif self.is_cache_artifact(ctx.artifact_path):
                entry = classify(GCCategory.ORPHAN_MANIFEST, 'artifact is missing')
            elif not self._same_path(ctx.manifest_path, manifest_path):
                ...
                entry = classify(GCCategory.ORPHAN_MIRROR, f"external artifact {self.describe_artifact_path(ctx.artifact_path)} is missing")
            # else: live mirror of an existing external artifact — keep silently
        rel = manifest_path.relative_to(self.cache_dir).as_posix()
    ...
    def _find_stale_dependency(
        ...
        for label, entry in dependencies.items():
            ...
            if entry.get('kind') == 'derivative':
                ...
                if manifest_path := entry.get('manifest'):
                    ...
                    if item is not None and entry.get('view') is None and isinstance(entry.get('dependencies'), dict):
                        child_node = self._nodes.get(entry.get('name', ''))
            ...
            if isinstance(nested, dict):
                found = self._find_stale_dependency(nested, scanned, deletable)
                if found is not None:
                    return found
        return None

    def _propagate_stale_dependencies(self, report: GCReport, scanned: dict[str, _ScannedManifest]) -> None:
        ...
        while changed:
            ...
            for rel, item in scanned.items():
                ...
                if item.entry is not None and item.entry.category is not GCCategory.UNVERIFIABLE:
                    continue
                stale_child = self._find_stale_dependency(item.manifest.dependencies, scanned, deletable)
    ...
    def _gc_remove_path(self, path: Path) -> bool:
        try:
            if path.is_dir():
                shutil.rmtree(path)
            else:
            ...
            self.log.warning("Cache GC: could not remove %s (%s)", path, error)
            return False
    ...
    def collect(
        ...
        """
        if report is None:
            report = self.scan_cache(revalidate=revalidate)
        ...
        for entry in sorted(report.deletable(), key=lambda entry: _GC_DELETE_PHASE.get(entry.category, 1)):
            ...
            for path in paths:
                if self._gc_remove_path(path):
                    n_removed += 1
                    touched_dirs.add(path.parent)
                    counts[entry.category.value] = counts.get(entry.category.value, 0) + 1
                    detail = f"; {entry.reason}" if entry.reason else ''
                    self.log.debug("Cache GC: removed %s (%s%s)", self.describe_artifact_path(path), entry.category.value, detail)
        for sidecar_path in report.disambiguation_sidecars:
            if not sidecar_path.exists():
                continue
            base_path = sidecar_path.with_name(sidecar_path.name[:-len(CACHE_DISAMBIGUATION_SUFFIX)])
            mapping = self._read_cache_disambiguation(base_path)
            keep = {digest: suffix for digest, suffix in mapping.items() if self._disambiguation_target_exists(base_path, suffix)}
            if keep == mapping and mapping:
                continue
            if keep:
                self._write_cache_disambiguation(base_path, keep)
                self.log.debug("Cache GC: pruned %i entries from %s", len(mapping) - len(keep), self.describe_artifact_path(sidecar_path))
            elif self._gc_remove_path(sidecar_path):
                n_removed += 1
                ...
                self.log.debug("Cache GC: removed %s (%s)", self.describe_artifact_path(sidecar_path), GCCategory.STALE_DISAMBIGUATION.value)
        for directory in sorted(touched_dirs, key=lambda path: len(path.parts), reverse=True):
            while directory != self.cache_dir and directory.is_relative_to(self.cache_dir):
                try:
        ...
        if n_removed:
            ...
            self.log.info("Cache GC: removed %i files (%s): %s", n_removed, _format_size(report.total_size()), summary)
        else:
            self.log.info("Cache GC: nothing to remove")
        return report
